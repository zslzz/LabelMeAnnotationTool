...
// Boolean indicating whether a scaling point is being edited:
    ...
    this.ShowScalingPoints = function () {
        ...
        for (var i = 0; i < this.x.length; i++) {
            this.scalepoints_ids.push(DrawPoint(this.dom_attach, this.x[i], this.y[i], 5, "#0000ff", "#ffffff", 2.5, this.scale));
        }
        for (var i = 0; i < this.scalepoints_ids.length; i++) $('#' + this.scalepoints_ids[i]).mousedown({
            obj: this,
            point: i
        }, function (e) {
    ...
    this.ShowControlPoints = function () {
        ...
        for (var i = 0; i < this.x.length; i++) {
            // Draw control point:
            this.control_ids.push(DrawPoint(this.dom_attach, this.x[i], this.y[i], 5, "#00ff00", "#ffffff", 2.5, this.scale));
    ...
    this.ShowCenterOfMass = function () {
        ...
        // Draw center point:
        this.center_id = DrawPoint(this.dom_attach, this.center_x, this.center_y, MarkerSize, "red", "#ffffff", MarkerSize / 2, this.scale);
    ...
    this.DrawPolygon = function (dom_id, x, y, obj_name, scale) {
        ...
        return DrawPolygon(dom_id, x, y, obj_name, 'none',HashObjectColor(obj_name),4, scale);
    };
}
